...
// import { setCurrentUserSP } from '../src/reducers/currentUser'
// import { setAlert, resetAlert } from '../src/reducers/alertNotif'
...
const PostPage = (props) => {
    ...
    const Posts = () => {
        return (
            <React.Fragment>
            {props.posts.map(p => (
                <Link href="/post/[name]" as={`/post/${encodeURIComponent(p.title)}`}>
                    <a>{encodeURIComponent(p.title)}</a>
                </Link>
            ))}
            </React.Fragment>
        )
